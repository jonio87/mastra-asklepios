...
export const ddxGeneratorTool = createTool({
  ...
  execute: async (inputData) => {
    ...
    // Auto-capture: persist each DDx entry as a hypothesis (fire-and-forget)
    persistDdxHypotheses(differentialDiagnosis).catch((err: unknown) => {
      logger.warn('Auto-capture of DDx hypotheses failed', {
...
/** Map DDx likelihood to probability range. */
function likelihoodToRange(likelihood: string): { low: number; high: number } {
  switch (likelihood) {
    case 'high':
    ...
    case 'dont-miss':
      return { low: 5, high: 50 };
    default:
      return { low: 10, high: 40 };
  }
}

/** Auto-persist DDx entries as hypothesis records to Layer 2B with dedup. */
async function persistDdxHypotheses(entries: DdxEntry[]): Promise<void> {
  ...
  for (const entry of entries) {
    const range = likelihoodToRange(entry.likelihood);
    const result = await store.addHypothesis({
      id: `hyp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      patientId: 'unknown',
      name: entry.diagnosis,
      date: today,
      ...(entry.icdCode ? { icdCode: entry.icdCode } : {}),
      probabilityLow: range.low,
      probabilityHigh: range.high,
      advocateCase: entry.supportingFeatures.join('; '),
      skepticCase: entry.contradictingFeatures?.join('; '),
      certaintyLevel: entry.likelihood === 'high' ? 'MODERATE' : 'WEAK',
      version: 1,
    });
  ...
  logger.info('Auto-captured DDx hypotheses', { inserted, duplicates });
}
